# DRAWING PROGRAM BY ABHRA2020-SMART
...
def showFileNav(op=False):
   #Op is short form for open as open is a key word
    ...
    window.withdraw()
    myFormats = [('Image File (*.txt)', '*.txt')]
    if op:
       filename = askopenfilename(title="Open Image",filetypes=myFormats) # Ask the user which file they want to open
    else:
       filename = asksaveasfilename(title="Save Image",filetypes=myFormats) # Ask the user choose a path to save their file to
...
label = Label(window, text='# Of Rows and Columns (Recommended # Of Rows and Columns is 25, 50): ')
...
submit = Button(window, text='Create New Image With Chosen Settings', command=onsubmit)
